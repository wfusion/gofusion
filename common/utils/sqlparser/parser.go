...
func (p *Parser) parseSelectStatement(compounded bool) (_ *SelectStatement, err error) {
	...
	// e.g. "LIMIT 1 OFFSET 2" or "LIMIT 1, 2" or "OFFSET 0 ROW FETCH NEXT/FIRST 1 ROWS/ROW ONLY"
	if !compounded && (p.peek() == LIMIT || p.peek() == OFFSET) {
		switch p.peek() {
		case LIMIT:
			p.lex()
		...
		case OFFSET:
			...
			if tok := p.peek(); tok != ROW && tok != ROWS {
				return &stmt, p.errorExpected(p.pos, p.tok, "ROW/ROWS")
			}

			p.lex()
			if tok := p.peek(); tok == FETCH {
				p.lex()
				if tok := p.peek(); tok != NEXT && tok != FIRST {
					return &stmt, p.errorExpected(p.pos, p.tok, "NEXT/FIRST")
				}

				p.lex()
				if stmt.Fetch, err = p.ParseExpr(); err != nil {
					return &stmt, err
				}
				if tok := p.peek(); tok != ROW && tok != ROWS {
					return &stmt, p.errorExpected(p.pos, p.tok, "ROW/ROWS")
				...
				p.lex()
				if tok := p.peek(); tok != ONLY {
					return &stmt, p.errorExpected(p.pos, p.tok, "ONLY")
				}
				p.lex()
			}
		}
