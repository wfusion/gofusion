...
func New[T any]() *Tree[T] { return NewFromMap[T](nil) }
...
func (t *Tree[T]) Get(s string) (interface{}, bool) {
	n := t.root
	search := s
	for {
		// Check for key exhaution
...
func (t *Tree[T]) LongestPrefix(s string) (string, interface{}, bool) {
	var last *leafNode[T]
	n := t.root
	search := s
	for {
...
// LongestCommonPrefix is like LongestPrefix, but instead of
// a longest prefix match, it will return the longest common
// prefix match.
func (t *Tree[T]) LongestCommonPrefix(s string) (string, bool) {
	var last *leafNode[T]
	n := t.root
	search := s
	for {
		...
		if strings.HasPrefix(search, n.prefix) {
			...
			return s[:len(s)+longestPrefix(search, n.prefix)-len(search)], true
		}
	}
	if last != nil {
...
func (t *Tree[T]) Minimum() (string, interface{}, bool) {
	n := t.root
	for {
		if n.isLeaf() {
...
func (t *Tree[T]) Maximum() (string, interface{}, bool) {
	n := t.root
	for {
