...
func (p RetryPublisher) Publish(ctx context.Context, topic string, messages ...*message.Message) error {
...
func (p RetryPublisher) send(ctx context.Context, topic string, msg *message.Message) error {
	var err error
	timeToNextRetry := p.config.TimeToFirstRetry

	for i := 0; i < p.config.MaxRetries; i++ {
		err = p.pub.Publish(ctx, topic, msg)
		if err == nil {
			return nil
		}
