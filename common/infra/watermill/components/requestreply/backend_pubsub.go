...
func (p *PubSubBackendConfig) Validate() error {
	...
	if p.Publisher == nil {
		err = multierr.Append(err, errors.New("publisher cannot be nil"))
	}
	if p.SubscriberConstructor == nil {
		err = multierr.Append(err, errors.New("subscriber constructor cannot be nil"))
	}
	if p.GeneratePublishTopic == nil {
		err = multierr.Append(err, errors.New("GeneratePublishTopic cannot be nil"))
	}
	if p.GenerateSubscribeTopic == nil {
		err = multierr.Append(err, errors.New("GenerateSubscribeTopic cannot be nil"))
	}
...
func (p PubSubBackend[Result]) OnCommandProcessed(ctx context.Context,
	params BackendOnCommandProcessedParams[Result]) error {
	...
	if err := p.config.Publisher.Publish(ctx, replyTopic, notificationMsg); err != nil {
