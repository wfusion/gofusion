...
type EventProcessorConfig struct {
	...
	// OnHandle works in a similar way to middlewares:
	// you can inject additional logic before and after handling a event.
	//
...
func (c EventProcessorConfig) Validate() error {
	...
	if c.Marshaler == nil {
		err = multierr.Append(err, errors.New("missing Marshaler"))
	}

	if c.GenerateSubscribeTopic == nil {
		err = multierr.Append(err, errors.New("missing GenerateHandlerTopic"))
	}
	if c.SubscriberConstructor == nil {
		err = multierr.Append(err, errors.New("missing SubscriberConstructor"))
	}
...
//
// It is required to call AddHandlersToRouter only if command processor
// is created with NewEventProcessor (disableRouterAutoAddHandlers is set to true).
...
func addHandlerToRouter(logger watermill.LoggerAdapter, r *message.Router, handlerName string, topicName string,
	handlerFunc message.NoPublishHandlerFunc, subscriber message.Subscriber) error {
...
func (p EventProcessor) routerHandlerFunc(handler EventHandler,
	logger watermill.LoggerAdapter) (message.NoPublishHandlerFunc, error) {
