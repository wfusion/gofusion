...
func (a DefaultMySQLOffsetsAdapter) AckMessageQuery(topic string, row Row,
	...
	ackQuery := `INSERT INTO ` + a.MessagesOffsetsTable(topic) + " (offset_consumed, offset_acked, consumer_group) " +
		...
		"offset_consumed=VALUES(offset_consumed), offset_acked=VALUES(offset_acked)"
	return ackQuery, []any{row.Offset, row.Offset, consumerGroup}
}
...
func (a DefaultMySQLOffsetsAdapter) ConsumedMessageQuery(topic string, row Row,
	...
	return ackQuery, []any{row.Offset, consumerGroup}
}
