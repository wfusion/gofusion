...
func NewSubscriber(
	...
	logger = logger.With(watermill.LogFields{
		"subscriber_uuid": utils.ShortUUID(),
	})
...
func (s *Subscriber) Subscribe(ctx context.Context, topic string) (<-chan *message.Message, error) {
	...
	logFields := watermill.LogFields{
		...
		"kafka_consumer_uuid": utils.ShortUUID(),
	}
	s.logger.Info("[Common] watermill subscribing to Kafka topic", logFields)
...
func (s *Subscriber) handleReconnects(
	...
	for {
		...
		if consumeClosed != nil {
			<-consumeClosed
			s.logger.Debug("[Common] watermill kafka consumeMessages stopped", logFields)
		} else {
			s.logger.Debug("[Common] watermill kafka empty consumeClosed", logFields)
		}
		...
		case <-s.closing:
			s.logger.Debug("[Common] watermill kafka closing subscriber, no reconnect needed", logFields)
			return
		case <-ctx.Done():
			s.logger.Debug("[Common] watermill kafka ctx cancelled, no reconnect needed", logFields)
			return
		default:
			s.logger.Debug("[Common] watermill kafka not closing, reconnecting", logFields)
		}

		s.logger.Info("[Common] watermill kafka reconnecting consumer", logFields)

		var err error
		consumeClosed, err = s.consumeMessages(ctx, topic, output, logFields)
		if err != nil {
			s.logger.Error("[Common] watermill kafka cannot reconnect messages consumer", err, logFields)
...
func (s *Subscriber) consumeMessages(
	...
	s.logger.Info("[Common] watermill kafka starting consuming", logFields)
	...
	go func() {
		select {
		case <-s.closing:
			s.logger.Debug("[Common] watermill kafka closing subscriber, cancelling consumeMessages", logFields)
			cancel()
		case <-ctx.Done():
	...
	if err != nil {
		s.logger.Debug(
			"[Common] watermill kafka starting consume failed, cancelling context",
	...
	go func() {
		...
		if err := client.Close(); err != nil {
			s.logger.Error("[Common] watermill kafka cannot close client", err, logFields)
		} else {
			s.logger.Debug("[Common] watermill kafka client closed", logFields)
		}
	}()
...
// tracer SaramaTracer,
	...
	go func() {
		defer func() {
			...
			if err := group.Close(); err != nil {
				s.logger.Error("[Common] watermill kafka group close with error", err, logFields)
			}

			s.logger.Info("[Common] watermill kafka consuming done", logFields)
			close(groupClosed)
		}()
	...
	ConsumeLoop:
		for {
			select {
			default:
				s.logger.Debug("[Common] watermill kafka not closing", logFields)
			case <-s.closing:
				s.logger.Debug("[Common] watermill kafka subscriber is closing, stopping group.Consume loop",
					logFields)
				break ConsumeLoop
			case <-ctx.Done():
				s.logger.Debug("[Common] watermill kafka ctx was cancelled, stopping group.Consume loop",
					logFields)
				break ConsumeLoop
			}
			...
			if err := group.Consume(ctx, []string{topic}, handler); err != nil {
				if err == sarama.ErrUnknown {
					...
					s.logger.Error("[Common] watermill kafka received unknown Sarama error", err, logFields)
				} else {
					s.logger.Error("[Common] watermill kafka group consume error", err, logFields)
				}
			...
			// see: https://github.com/wfusion/gofusion/common/infra/watermill/issues/210
			s.logger.Debug("[Common] watermill kafka consume stopped without any error, running consume again",
				logFields)
		}
	}()
	...
	go func() {
		...
		for {
			...
			case err, ok := <-group.Errors():
				...
				s.logger.Error("[Common] watermill kafka sarama internal error", err, logFields)
			case <-ctx.Done():
				return
			}
		}
...
// tracer SaramaTracer,
	...
	if err != nil {
		return nil, errors.Wrap(err, "cannot create consumer client")
	}
	...
	partitionConsumersWg := new(sync.WaitGroup)
	...
	for _, partition := range partitions {
		...
		partitionConsumer, err := consumer.ConsumePartition(topic,
			partition, s.config.OverwriteSaramaConfig.Consumer.Offsets.Initial)
		if err != nil {
			if err := client.Close(); err != nil && err != sarama.ErrClosedClient {
				s.logger.Error("[Common] watermill kafka cannot close client", err, partitionLogFields)
			}
...
func (s *Subscriber) consumePartition(
	...
	defer func() {
		if err := partitionConsumer.Close(); err != nil {
			s.logger.Error("[Common] watermill kafka cannot close partition consumer", err, logFields)
		}
		partitionConsumersWg.Done()
		s.logger.Debug("[Common] watermill kafka consumePartition stopped", logFields)
	}()
	...
	for {
		...
		case kafkaMsg := <-kafkaMessages:
			if kafkaMsg == nil {
				s.logger.Debug("[Common] watermill kafkaMsg is closed, stopping consumePartition", logFields)
				return
			}
		...
		case <-s.closing:
			s.logger.Debug("[Common] watermill kafka subscriber is closing, stopping consumePartition", logFields)
			return

		case <-ctx.Done():
			s.logger.Debug("[Common] watermill kafka ctx was cancelled, stopping consumePartition", logFields)
			return
		}
	}
}
...
func (s *Subscriber) createMessagesHandler(output chan *message.Message) messageHandler {
	return messageHandler{
		...
		saramaCfg:       s.config.OverwriteSaramaConfig,
...
func (h *consumerGroupHandler) Setup(sess sarama.ConsumerGroupSession) error {
	h.logger.Trace(
		fmt.Sprintf("[Common] watermill kafka get cluster member(%s) generation(%v)",
			sess.MemberID(), sess.GenerationID()),
		h.messageLogFields.Copy(),
	)
	h.messageAccepted()
	return nil
}
...
func (h *consumerGroupHandler) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	...
	for kafkaMsg := range claim.Messages() {
		h.logger.Debug("[Common] watermill kafka message claimed", logFields)
		if err := h.messageHandler.processMessage(h.ctx, kafkaMsg, sess, logFields); err != nil {
			return err
		}
		select {
		case <-h.closing:
			h.logger.Debug("[Common] watermill kafka subscriber is closing, stopping consumerGroupHandler",
				logFields)
			return nil

		case <-h.ctx.Done():
			h.logger.Debug("[Common] watermill kafka ctx was cancelled, stopping consumerGroupHandler", logFields)
			return nil
		default:
			continue
		}
...
func (h *consumerGroupHandler) messageAccepted() {
	select {
	case _, ok := <-h.running:
		if !ok {
			return
...
func (h messageHandler) processMessage(
	...
	rawMessageID := fmt.Sprintf("partition(%v)_offset(%v)", kafkaMsg.Partition, kafkaMsg.Offset)
	...
	ctx = context.WithValue(ctx, watermill.ContextKeyMessageUUID, msg.UUID)
	...
	// check session has been canceled after re-balancing
...
ResendLoop:
	for {
		...
		case h.outputChannel <- msg:
			h.logger.Trace("[Common] watermill kafka message sent to consumer", receivedMsgLogFields)
		case <-h.closing:
			h.logger.Trace("[Common] watermill kafka closing, message discarded", receivedMsgLogFields)
			return nil
		case <-ctx.Done():
			h.logger.Trace("[Common] watermill kafka closing, ctx cancelled before sent to consumer",
				receivedMsgLogFields)
			return nil
		case <-sessionContext.Done():
			h.logger.Trace("[Common] watermill kafka closing, session ctx cancelled before sent to consumer",
				receivedMsgLogFields)
			return nil
		}
		...
		case <-msg.Acked():
			if sess != nil {
				...
				if !h.saramaCfg.Consumer.Offsets.AutoCommit.Enable {
					sess.Commit()
				}
			}
			h.logger.Trace("[Common] watermill message acked", receivedMsgLogFields)
			break ResendLoop
		case <-msg.Nacked():
			h.logger.Trace("[Common] watermill message nacked", receivedMsgLogFields)
			...
			continue ResendLoop
		case <-h.closing:
			h.logger.Trace("[Common] watermill kafka closing, message discarded before ack", receivedMsgLogFields)
			return nil
		case <-ctx.Done():
			h.logger.Trace("[Common] watermill kafka closing, ctx cancelled before ack", receivedMsgLogFields)
			return nil
		case <-sessionContext.Done():
			h.logger.Trace("[Common] watermill kafka closing, session ctx cancelled before ack", receivedMsgLogFields)
			return nil
		}
	}
...
func (s *Subscriber) SubscribeInitialize(topic string) (err error) {
	...
	s.logger.Info("[Common] watermill kafka created Kafka topic", watermill.LogFields{"topic": topic})
...
func (s *Subscriber) PartitionOffset(topic string) (PartitionOffset, error) {
	...
	for _, partition := range partitions {
		offset, err := client.GetOffset(topic, partition, s.config.OverwriteSaramaConfig.Consumer.Offsets.Initial)
		if err != nil {
			return nil, err
		}
