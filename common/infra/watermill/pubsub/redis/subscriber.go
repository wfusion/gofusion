...
func (s *Subscriber) Subscribe(ctx context.Context, topic string) (<-chan *message.Message, error) {
	...
	s.logger.Info("[Common] watermill redis subscribing to redis stream topic", logFields)
...
func (s *Subscriber) consumeMessages(ctx context.Context, topic string,
	output chan *message.Message, logFields watermill.LogFields) (consumeMessageClosed chan struct{}, err error) {
	...
	go func() {
		select {
		case <-s.closing:
			s.logger.Debug("[Common] watermill redis closing subscriber, cancelling consumeMessages", logFields)
			cancel()
		case <-ctx.Done():
	...
	if s.config.ConsumerGroup != "" {
		...
		if _, err := s.client.XGroupCreateMkStream(ctx, topic,
			s.config.ConsumerGroup, s.config.OldestId).Result(); err != nil && err.Error() != redisBusyGroup {
			return nil, err
		}
	}
	...
	if err != nil {
		s.logger.Debug(
			"[Common] watermill redis starting consume failed, cancelling context",
...
func (s *Subscriber) consumeStreams(ctx context.Context, stream string,
	output chan *message.Message, logFields watermill.LogFields) (chan struct{}, error) {
	...
	go func() {
		...
		for {
			...
			case xs := <-readChannel:
				if xs == nil {
					s.logger.Debug(
						"[Common] watermill redis readStreamChannel is closed, stopping readStream", logFields)
					return
				}
				if err := messageHandler.processMessage(ctx, xs.Stream, &xs.Messages[0], logFields); err != nil {
					s.logger.Error("[Common] watermill redis processMessage fail", err, logFields)
					return
				}
			case <-s.closing:
				s.logger.Debug("[Common] watermill redis subscriber is closing, stopping readStream", logFields)
				return
			case <-ctx.Done():
				s.logger.Debug("[Common] watermill redis ctx was cancelled, stopping readStream", logFields)
				return
			}
		}
	}()
...
func (s *Subscriber) read(ctx context.Context, stream string,
	readChannel chan<- *redis.XStream, logFields watermill.LogFields) {
	wg := &sync.WaitGroup{}
	...
	for {
		...
		default:
			...
			if errors.Is(err, redis.Nil) {
				...
				if _, ok := utils.IsChannelClosed(s.closing); !ok &&
					!errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
					s.logger.Error("[Common] watermill redis read fail", err, logFields)
				}
			}
...
func (s *Subscriber) claim(ctx context.Context, stream string,
	readChannel chan<- *redis.XStream, keep bool, wg *sync.WaitGroup, logFields watermill.LogFields) {
	var (
...
OUTER_LOOP:
	for {
		...
		if err != nil {
			s.logger.Error(
				"[Common] watermill redis xpendingext fail",
				err,
				logFields,
			)
			continue
		...
		for _, xp = range xps {
			...
			if shouldClaim {
				...
				if err != nil {
					s.logger.Error(
						"[Common] watermill redis xclaim fail",
						err,
...
func (s *Subscriber) checkConsumers(ctx context.Context, stream string,
	wg *sync.WaitGroup, logFields watermill.LogFields) {
	...
	for {
		...
		if err != nil {
			s.logger.Error(
				"[Common] watermill redis xinfoconsumers failed",
				err,
				logFields,
			)
		}
		for _, xic := range xics {
			...
			if xic.Pending == 0 {
				if err = s.client.XGroupDelConsumer(ctx, stream, s.config.ConsumerGroup, xic.Name).Err(); err != nil {
					s.logger.Error(
						"[Common] watermill redis xgroupdelconsumer failed",
						err,
						logFields,
					)
				}
...
func (s *Subscriber) Close() error {
	...
	if !s.config.DisableRedisConnClose {
		if err := s.client.Close(); err != nil {
			return err
		}
	}

	s.logger.Debug("[Common] watermill redis stream subscriber closed", nil)
...
func (h *messageHandler) processMessage(ctx context.Context, stream string,
	xm *redis.XMessage, messageLogFields watermill.LogFields) error {
	receivedMsgLogFields := messageLogFields.Add(watermill.LogFields{
		"xadd_id":        xm.ID,
		"stream":         stream,
		"message_raw_id": xm.ID,
	})

	h.logger.Trace("[Common] watermill received message from redis stream", receivedMsgLogFields)
	...
	ctx = context.WithValue(ctx, watermill.ContextKeyRawMessageID, xm.ID)
...
ResendLoop:
	for {
		...
		case h.outputChannel <- msg:
			h.logger.Trace("[Common] watermill redis message sent to consumer", receivedMsgLogFields)
		case <-h.closing:
			h.logger.Trace("[Common] watermill redis closing, message discarded", receivedMsgLogFields)
			return nil
		case <-ctx.Done():
			h.logger.Trace("[Common] watermill redis closing, ctx cancelled before sent to consumer",
		...
		case <-msg.Acked():
			if h.consumerGroup != "" {
				...
				if err != nil {
					h.logger.Error("[Common] watermill redis message acked fail", err, receivedMsgLogFields)
				}
			}
			h.logger.Trace("[Common] watermill redis message acked", receivedMsgLogFields)
			break ResendLoop
		case <-msg.Nacked():
			h.logger.Trace("[Common] watermill redis message nacked", receivedMsgLogFields)
			...
			continue ResendLoop
		case <-h.closing:
			h.logger.Trace("[Common] watermill redis closing, message discarded before ack", receivedMsgLogFields)
			return nil
		case <-ctx.Done():
			h.logger.Trace("[Common] watermill redis closing, ctx cancelled before ack", receivedMsgLogFields)
			return nil
		}
	}
