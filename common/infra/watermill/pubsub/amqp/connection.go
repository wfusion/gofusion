...
func (c *ConnectionWrapper) Close() error {
	...
	c.logger.Info("[Common] watermill closing AMQP pub/sub", nil)
	defer c.logger.Info("[Common] watermill amqp closed AMQP pub/sub", nil)

	c.connectionWaitGroup.Wait()

	if err := c.amqpConnection.Close(); err != nil {
		c.logger.Error("[Common] watermill amqp connection close error", err, nil)
	}
...
func (c *ConnectionWrapper) connect() error {
	...
	c.logger.Info("[Common] watermill connected to AMQP", nil)
...
func (c *ConnectionWrapper) handleConnectionClose() {
	for {
		c.logger.Debug("[Common] watermill amqp handle connection close is waiting for c.connected", nil)
		<-c.connected
		c.logger.Debug("[Common] watermill amqp handle connection close is for connection or pub/sub close", nil)

		notifyCloseConnection := c.amqpConnection.NotifyClose(make(chan *amqp.Error))

		select {
		case <-c.closing:
			c.logger.Debug("[Common] watermill amqp stopping handle connection close", nil)
			c.connected = make(chan struct{})
			return
		case err := <-notifyCloseConnection:
			c.connected = make(chan struct{})
			c.logger.Error("[Common] watermill received close notification from AMQP, reconnecting", err, nil)
			c.reconnect()
		}
	}
}
...
func (c *ConnectionWrapper) reconnect() {
	...
	if err := backoff.Retry(func() error {
		...
		c.logger.Error("[Common] watermill cannot reconnect to AMQP, retrying", err, nil)
		...
		c.logger.Error("[Common] watermill amqp reconnect failed", err, nil)
	}
}
