...
func NewSubscriber(config Config, logger watermill.LoggerAdapter) (*Subscriber, error) {
	...
	closeSubscriber := func() error {
		...
		logger.Debug("[Common] watermill amqp closing subscriber", nil)

		close(closedChan)

		subscriberWaitGroup.Wait()

		logger.Debug("[Common] watermill amqp closing connection", nil)
...
func (s *Subscriber) Subscribe(ctx context.Context, topic string) (<-chan *message.Message, error) {
	...
	go func(ctx context.Context) {
		defer func() {
			close(out)
			s.logger.Info("[Common] watermill stopped consuming from AMQP channel", logFields)
	...
	ReconnectLoop:
		for {
			s.logger.Debug("[Common] watermill amqp waiting for s.connected or s.closing in reconnect loop",
				logFields)

			// to avoid race conditions with <-s.connected
			select {
			case <-s.closing:
				s.logger.Debug("[Common] watermill amqp stopping reconnect loop (already closing)", logFields)
				break ReconnectLoop
			case <-s.closedChan:
				s.logger.Debug("[Common] watermill amqp stopping reconnect loop (subscriber closing)", logFields)
				break ReconnectLoop
			default:
				// not closing yet
			}
			...
			select {
			case <-s.connected:
				s.logger.Debug("[Common] watermill amqp connection established in ReconnectLoop", logFields)
				// runSubscriber blocks until connection fails or Close() is called
				s.runSubscriber(ctx, out, queueName, exchangeName, logFields)
			case <-s.closing:
				s.logger.Debug("[Common] watermill amqp stopping reconnect loop (closing)", logFields)
				break ReconnectLoop
			case <-ctx.Done():
				s.logger.Debug("[Common] watermill amqp stopping reconnect loop (ctx done)", logFields)
				break ReconnectLoop
			}
...
func (s *Subscriber) SubscribeInitialize(topic string) (err error) {
	...
	s.logger.Info("[Common] watermill amqp initializing subscribe", logFields)
...
func (s *Subscriber) prepareConsume(queueName string, exchangeName string, logFields watermill.LogFields) (err error) {
	...
	defer func() {
		if channelCloseErr := channel.Close(); channelCloseErr != nil {
			err = multierr.Append(err, channelCloseErr)
		}
	}()
	...
	s.logger.Debug("[Common] watermill amqp queue bound to exchange", logFields)
...
func (s *Subscriber) runSubscriber(
	...
	channel, err := s.openSubscribeChannel(logFields)
	if err != nil {
		s.logger.Error("[Common] watermill amqp failed to open channel", err, logFields)
		return
	}
	defer func() {
		if err := channel.Close(); err != nil {
			s.logger.Error("[Common] watermill amqp failed to close channel", err, logFields)
		}
	}()
	...
	s.logger.Info("[Common] watermill starting consuming from AMQP channel", logFields)
...
func (s *Subscriber) openSubscribeChannel(logFields watermill.LogFields) (*amqp.Channel, error) {
	...
	s.logger.Debug("[Common] watermill amqp channel opened", logFields)
	...
	if s.config.Consume.Qos != (QosConfig{}) {
		...
		s.logger.Debug("[Common] watermill amqp qos set", logFields)
	}
...
func (s *subscription) ProcessMessages(ctx context.Context) {
	...
	if err != nil {
		s.logger.Error("[Common] watermill amqp failed to start consuming messages", err, s.logFields)
		return
	}
	...
	for {
		...
		case amqpMsg := <-amqpMsgs:
			if err := s.processMessage(ctx, amqpMsg, s.out, s.logFields); err != nil {
				s.logger.Error("[Common] watermill amqp processing message failed, sending nack", err, s.logFields)

				if err := s.nackMsg(amqpMsg); err != nil {
					s.logger.Error("[Common] watermill amqp cannot nack message", err, s.logFields)
		...
		case <-s.notifyCloseChannel:
			s.logger.Error("[Common] watermill amqp channel closed, stopping process messages", nil, s.logFields)
			break ConsumingLoop

		case <-s.closing:
			s.logger.Info("[Common] watermill amqp closing from subscriber received", s.logFields)
			break ConsumingLoop

		case <-s.closedChan:
			s.logger.Info("[Common] watermill amqp subscriber closed", s.logFields)
			break ConsumingLoop

		case <-ctx.Done():
			s.logger.Info("[Common] watermill amqp closing from ctx received", s.logFields)
			break ConsumingLoop
		}
	}
}
...
func (s *subscription) processMessage(
	...
	ctx = context.WithValue(ctx, watermill.ContextKeyRawMessageID, amqpMsg.MessageId)
	ctx, cancelCtx := context.WithCancel(ctx)
	defer cancelCtx()
	msg.Metadata[watermill.ContextKeyMessageUUID] = msg.UUID
	msg.Metadata[watermill.ContextKeyRawMessageID] = amqpMsg.MessageId
	msg.SetContext(ctx)

	msgLogFields := logFields.Add(watermill.LogFields{
		"message_raw_id": amqpMsg.MessageId,
		"message_uuid":   msg.UUID,
	})
	s.logger.Trace("[Common] watermill amqp unmarshaled message", msgLogFields)

	select {
	case <-s.closing:
		s.logger.Info("[Common] watermill amqp message not consumed, pub/sub is closing", msgLogFields)
		return s.nackMsg(amqpMsg)
	case <-s.closedChan:
		s.logger.Info("[Common] watermill amqp message not consumed, subscriber is closed", msgLogFields)
		return s.nackMsg(amqpMsg)
	case out <- msg:
		s.logger.Trace("[Common] watermill amqp message sent to consumer", msgLogFields)
	}

	select {
	case <-s.closing:
		s.logger.Trace("[Common] watermill amqp closing pub/sub, message discarded before ack", msgLogFields)
		return s.nackMsg(amqpMsg)
	case <-s.closedChan:
		s.logger.Info("[Common] watermill amqp message not consumed, subscriber is closed", msgLogFields)
		return s.nackMsg(amqpMsg)
	case <-msg.Acked():
		s.logger.Trace("[Common] watermill amqp message acked", msgLogFields)
		return amqpMsg.Ack(false)
	case <-msg.Nacked():
		s.logger.Trace("[Common] watermill amqp message nacked", msgLogFields)
		return s.nackMsg(amqpMsg)
	}
}
