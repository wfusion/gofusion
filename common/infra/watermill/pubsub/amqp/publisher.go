...
func NewPublisher(config Config, logger watermill.LoggerAdapter) (*Publisher, error) {
	...
	chanProvider, err := newChannelProvider(conn, config.Publish.ChannelPoolSize,
		config.Publish.ConfirmDelivery, logger)
	if err != nil {
	...
	closePublisher := func() error {
		logger.Debug("[Common] watermill amqp closing publisher connection", nil)
...
func NewPublisherWithConnection(config Config, logger watermill.LoggerAdapter,
	...
	chanProvider, err := newChannelProvider(conn,
		config.Publish.ChannelPoolSize, config.Publish.ConfirmDelivery, logger)
	if err != nil {
		return nil, fmt.Errorf("create new channel pool: %w", err)
	}
	...
	closePublisher := func() error {
		logger.Debug("[Common] watermill amqp publisher closed", nil)
...
func (p *Publisher) Publish(ctx context.Context, topic string, messages ...*message.Message) (err error) {
	...
	for _, msg := range messages {
		if err := p.publishMessage(ctx, exchangeName, routingKey, msg, c, logFields); err != nil {
			return err
		}
	}
...
func (p *Publisher) publishMessage(
	...
	p.logger.Trace("[Common] watermill amqp publishing message", logFields)
	...
	if !channel.DeliveryConfirmationEnabled() {
		p.logger.Trace("[Common] watermill amqp message published", logFields)

		return nil
	}

	p.logger.Trace("[Common] watermill message published. waiting for delivery confirmation.", logFields)

	if !channel.Delivered() {
		return fmt.Errorf("delivery not confirmed for message [%s]", msg.UUID)
	}

	p.logger.Trace("[Common] watermill delivery confirmed for message", logFields)
...
func (p *Publisher) preparePublishBindings(topic string, channel *amqp.Channel) error {
	...
	if p.config.Exchange.GenerateName(topic) != "" {
		if err := p.config.TopologyBuilder.ExchangeDeclare(
			channel, p.config.Exchange.GenerateName(topic), p.config); err != nil {
			return err
		}
	}
