...
func (c Config) validate(validateConnection bool) error {
	...
	if validateConnection {
		if c.Connection.AmqpURI == "" {
			err = multierr.Append(err, errors.New("empty Config.AmqpURI"))
		}
	}
	if c.Marshaler == nil {
		err = multierr.Append(err, errors.New("missing Config.Marshaler"))
	}
	if c.Exchange.GenerateName == nil {
		err = multierr.Append(err, errors.New("missing Config.GenerateName"))
	}
...
func (c Config) validatePublisher(validateConnection bool) error {
	...
	if c.Publish.GenerateRoutingKey == nil {
		err = multierr.Append(err, errors.New("missing Config.GenerateRoutingKey"))
	}
...
func (c Config) validateSubscriber(validateConnection bool) error {
	...
	if c.Queue.GenerateName == nil {
		err = multierr.Append(err, errors.New("missing Config.Queue.GenerateName"))
	}
