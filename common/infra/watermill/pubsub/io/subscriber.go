...
func (s *Subscriber) consume(ctx context.Context, topic string, output chan *message.Message) {
	...
	ResendLoop:
		for {
			...
			select {
			case <-msg.Acked():
				logger.Trace("[Common] watermill message acked", nil)
				break ResendLoop
			case <-msg.Nacked():
				logger.Trace("[Common] watermill message nacked, resending", nil)
				msg = msg.Copy()
				continue ResendLoop
			case <-ctx.Done():
				logger.Info("[Common] watermill context closed without ack", nil)
				return
			case <-s.closing:
				logger.Info("[Common] watermill subscriber closed without ack", nil)
				return
			}
		}
	}
