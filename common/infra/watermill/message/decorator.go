...
func (d messageTransformPublisherDecorator) Publish(ctx context.Context, topic string, messages ...*Message) error {
	for i := range messages {
		d.transform(messages[i])
	}
	return d.Publisher.Publish(ctx, topic, messages...)
}
