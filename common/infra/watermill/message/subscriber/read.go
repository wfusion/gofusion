...
// BulkRead reads provided amount of messages from the provided channel,
// until a timeout occurrs or the limit is reached.
func BulkRead(messagesCh <-chan *message.Message, limit int,
	timeout time.Duration) (receivedMessages message.Messages, all bool) {
MessagesLoop:
...
// until a timeout occurrs or the limit is reached.
func BulkReadWithDeduplication(messagesCh <-chan *message.Message, limit int,
	timeout time.Duration) (receivedMessages message.Messages, all bool) {
