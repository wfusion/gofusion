...
import (
	...
	"github.com/wfusion/gofusion/common/infra/watermill/pkg"
...
var (
	...
	ErrRouterIsAlreadyRunning     = errors.New("router is already running")
)
...
func (r *Router) AddMiddleware(m ...HandlerMiddleware) {
	r.logger.Debug("[Common] watermill adding middleware",
		watermill.LogFields{"count": fmt.Sprintf("%d", len(m))})
...
func (r *Router) AddHandler(
	...
	r.logger.Info("[Common] watermill adding handler", watermill.LogFields{
	...
	publisherName, subscriberName := pkg.StructName(publisher), pkg.StructName(subscriber)
...
func (r *Router) Run(ctx context.Context) (err error) {
	if r.isRunning {
		return ErrRouterIsAlreadyRunning
	}
	r.isRunning = true
	...
	r.logger.Debug("[Common] watermill loading plugins", nil)
	...
	r.logger.Info("[Common] watermill waiting for messages", watermill.LogFields{
		"timeout": r.config.CloseTimeout,
	})

	<-r.ClosedCh

	r.logger.Info("[Common] watermill all messages processed", nil)
...
func (r *Router) RunHandlers(ctx context.Context) error {
	...
	r.logger.Info("[Common] watermill running router handlers", watermill.LogFields{"count": len(r.handlers)})
	...
	for name, h := range r.handlers {
		...
		r.logger.Debug("[Common] watermill subscribing to topic", watermill.LogFields{
		...
		go func() {
			...
			r.handlersWg.Done()
			r.logger.Info("[Common] watermill subscriber stopped", watermill.LogFields{
				"subscriber_name": h.name,
				"topic":           h.subscribeTopic,
			})
...
func (r *Router) closeWhenAllHandlersStopped(ctx context.Context) {
	...
	case <-ctx.Done():
	default:
		r.logger.Error("[Common] watermill all handlers stopped, closing router",
			errors.New("all router handlers stopped"), nil)
	}

	if err := r.Close(); err != nil {
		r.logger.Error("[Common] watermill cannot close router", err, nil)
	}
}
...
func (r *Router) Close() error {
	...
	r.logger.Info("[Common] watermill closing router", nil)
	defer r.logger.Info("[Common] watermill router closed", nil)
...
func (r *Router) waitForHandlers() bool {
	...
	return utils.Timeout(r.config.CloseTimeout, utils.TimeoutWg(&waitGroup))
}
...
func (h *handler) run(ctx context.Context, middlewares []middleware) {
	h.logger.Info("[Common] watermill starting handler", watermill.LogFields{
		"subscriber_name": h.name,
		"topic":           h.subscribeTopic,
	})
	...
	if h.publisher != nil {
		h.logger.Debug("[Common] watermill waiting for publisher to close", nil)
		if err := h.publisher.Close(); err != nil {
			h.logger.Error("[Common] watermill failed to close publisher", err, nil)
		}
		h.logger.Debug("[Common] watermill publisher closed", nil)
	}

	h.logger.Debug("[Common] watermill router handler stopped", nil)
	close(h.stopped)
}
...
func (h *Handler) AddMiddleware(m ...HandlerMiddleware) {
	handler := h.handler
	handler.logger.Debug("[Common] watermill adding middleware to handler", watermill.LogFields{
...
func (h *handler) handleClose(ctx context.Context) {
	...
	case <-h.routersCloseCh:
		...
		h.logger.Debug("[Common] watermill waiting for subscriber to close", nil)
		if err := h.subscriber.Close(); err != nil {
			h.logger.Error("[Common] watermill failed to close subscriber", err, nil)
		}
		h.logger.Debug("[Common] watermill subscriber closed", nil)
	case <-ctx.Done():
...
func (h *handler) handleMessage(msg *Message, handler HandlerFunc) {
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			h.logger.Error(
				"[Common] watermill panic recovered in handler. Stack: "+string(debug.Stack()),
	...
	h.logger.Trace("[Common] watermill received message", msgFields)

	producedMessages, err := handler(msg)
	if err != nil {
		h.logger.Error("[Common] watermill handler returned error", err, nil)
		msg.Nack()
		return
	}

	// ack & nack by business for non-router calling
	disableAck := false
	if len(producedMessages) > 0 {
		for i, msg := range producedMessages {
			if _, disableAck = msg.Metadata[watermill.MessageRouterAck]; disableAck {
				producedMessages = append(producedMessages[0:i], producedMessages[i+1:]...)
			}
		}
	}
	...
	if err := h.publishProducedMessages(msg.Context(), producedMessages, msgFields); err != nil {
		h.logger.Error("[Common] watermill publishing produced messages failed", err, nil)
		if !disableAck {
			msg.Nack()
		} else {
			h.logger.Trace("[Common] watermill message should be nack by business", msgFields)
		}
		return
	}
	if !disableAck {
		msg.Ack()
		h.logger.Trace("[Common] watermill message acked", msgFields)
	} else {
		h.logger.Trace("[Common] watermill message should be acked by business", msgFields)
	}
}

func (h *handler) publishProducedMessages(ctx context.Context,
	producedMessages Messages, msgFields watermill.LogFields) error {
	if len(producedMessages) == 0 {
		return nil
	}
	...
	for _, msg := range producedMessages {
		if err := h.publisher.Publish(ctx, h.publishTopic, msg); err != nil {
...
func (disabledPublisher) Publish(ctx context.Context, topic string, messages ...*Message) error {
	return ErrOutputInNoPublisherHandler
}
