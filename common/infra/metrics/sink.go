...
type MetricSink interface {
	...
	SetGauge(key []string, val float32, opts ...utils.OptionExtender)
	SetGaugeWithLabels(key []string, val float32, labels []Label, opts ...utils.OptionExtender)

	// EmitKey Should emit a Key/Value pair for each call
	EmitKey(key []string, val float32, opts ...utils.OptionExtender)

	// IncrCounter should accumulate values
	IncrCounter(key []string, val float32, opts ...utils.OptionExtender)
	IncrCounterWithLabels(key []string, val float32, labels []Label, opts ...utils.OptionExtender)

	// AddSample are for timing information, where quantiles are used
	AddSample(key []string, val float32, opts ...utils.OptionExtender)
	AddSampleWithLabels(key []string, val float32, labels []Label, opts ...utils.OptionExtender)
}
...
type PrecisionGaugeMetricSink interface {
	SetPrecisionGauge(key []string, val float64, opts ...utils.OptionExtender)
	SetPrecisionGaugeWithLabels(key []string, val float64, labels []Label, opts ...utils.OptionExtender)
}

// PrecisionSampleMetricSink interface is used to support 64 bit precisions for Sinks, if needed.
type PrecisionSampleMetricSink interface {
	AddPrecisionSample(key []string, val float64, opts ...utils.OptionExtender)
	AddPrecisionSampleWithLabels(key []string, val float64, labels []Label, opts ...utils.OptionExtender)
}
...
func (fh FanoutSink) SetPrecisionGaugeWithLabels(key []string, val float64, labels []Label,
	opts ...utils.OptionExtender) {
	for _, s := range fh {
		// The Sink needs to implement PrecisionGaugeMetricSink,
		// in case it doesn't, the metric value won't be set and ignored instead
