...
func Bucket(buckets []float64) utils.OptionFunc[metrics.Option] {
	return func(o *metrics.Option) {
		o.Buckets = buckets
	}
}
...
type PrometheusOpts struct {
	...
	// Gauges, Summaries, Histograms, and Counters allow us to pre-declare metrics by giving
...
// HistogramDefinition can be provided to PrometheusOpts to declare a constant summary that is not deleted on expiry.
...
func (p *PrometheusSink) collectAtTime(c chan<- prometheus.Metric, t time.Time) {
	...
	p.histograms.Range(func(k, v interface{}) bool {
		...
		h := v.(*histogram)
		lastUpdate := h.updatedAt
		if expire && lastUpdate.Add(p.expiration).Before(t) {
			if h.canDelete {
				p.histograms.Delete(k)
				return true
			}
		}
		h.Collect(c)
...
func initSummaries(m *sync.Map, summaries []SummaryDefinition, help map[string]string) {
	for _, s := range summaries {
		...
		pS := prometheus.NewSummary(prometheus.SummaryOpts{
			...
			Objectives:  clone.Clone(objectives),
		})
		m.Store(hash, &summary{Summary: pS})
	}
}

func initHistograms(m *sync.Map, summaries []HistogramDefinition, help map[string]string) {
	for _, s := range summaries {
		key, hash := flattenKey(s.Name, s.ConstLabels)
		help[fmt.Sprintf("summary.%s", key)] = s.Help
		pS := prometheus.NewHistogram(prometheus.HistogramOpts{
			Name:        key,
			Help:        s.Help,
			ConstLabels: prometheusLabels(s.ConstLabels),
			Buckets:     clone.Clone(defaultBuckets),
		})
		m.Store(hash, &summary{Summary: pS})
	}
}
...
func (p *PrometheusSink) SetGauge(parts []string, val float32, opts ...utils.OptionExtender) {
...
func (p *PrometheusSink) SetPrecisionGauge(parts []string, val float64, opts ...utils.OptionExtender) {
...
func (p *PrometheusSink) AddSample(parts []string, val float32, opts ...utils.OptionExtender) {
	p.AddSampleWithLabels(parts, val, nil)
}

func (p *PrometheusSink) AddSampleWithLabels(parts []string, val float32, labels []metrics.Label,
	opts ...utils.OptionExtender) {
	opt := utils.ApplyOptions[metrics.Option](opts...)
	if opt.Precision {
		p.AddPrecisionSampleWithLabels(parts, float64(val), labels, opts...)
		return
	}

	buckets := defaultBuckets
	if len(opt.Buckets) > 0 {
		buckets = clone.Clone(opt.Buckets)
	}

	key, hash := flattenKey(parts, labels)
	ps, ok := p.histograms.Load(hash)

	// Does the summary already exist for this sample type?
	if ok {
		localHistogram := *ps.(*histogram)
		localHistogram.Observe(float64(val))
		localHistogram.updatedAt = time.Now()
		p.histograms.Store(hash, &localHistogram)

		// The summary does not exist, create the Summary and allow it to be deleted
	} else {
		help := key
		existingHelp, ok := p.help[fmt.Sprintf("histogram.%s", key)]
		if ok {
			help = existingHelp
		}
		...
		p.histograms.Store(hash, ps)
	}
}

func (p *PrometheusSink) AddPrecisionSample(parts []string, val float64, opts ...utils.OptionExtender) {
	p.AddPrecisionSampleWithLabels(parts, val, nil, opts...)
}

func (p *PrometheusSink) AddPrecisionSampleWithLabels(parts []string, val float64, labels []metrics.Label,
	opts ...utils.OptionExtender) {
	key, hash := flattenKey(parts, labels)
...
func (p *PrometheusSink) EmitKey(key []string, val float32, opts ...utils.OptionExtender) {
	panic(errors.New("prometheus sink not implement EmitKey"))
}

func (p *PrometheusSink) IncrCounter(parts []string, val float32, opts ...utils.OptionExtender) {
...
func (s *PrometheusPushSink) Shutdown() {
	if _, ok := utils.IsChannelClosed(s.stopChan); ok {
		return
	}
