...
// setting the `propagateHostname` flag ensures that `s.HostName`
// overrides the host tag naively set by the DogStatsd server
...
func (s *DogStatsdSink) parseKey(key []string) ([]string, []metrics.Label) {
	// Since DogStatsd supports dimensionality via tags on metric keys,
	// this sink's approach is to splice the hostname out of the key in favor of a `host` tag
...
func (s *DogStatsdSink) SetGauge(key []string, val float32, opts ...utils.OptionExtender) {
	s.SetGaugeWithLabels(key, val, nil)
}

func (s *DogStatsdSink) SetPrecisionGauge(key []string, val float64, opts ...utils.OptionExtender) {
	s.SetPrecisionGaugeWithLabels(key, val, nil)
}

func (s *DogStatsdSink) IncrCounter(key []string, val float32, opts ...utils.OptionExtender) {
	s.IncrCounterWithLabels(key, val, nil)
}

// EmitKey is not implemented since DogStatsd does not provide a metric type that holds an
// arbitrary number of values
func (s *DogStatsdSink) EmitKey(key []string, val float32, opts ...utils.OptionExtender) {
}

func (s *DogStatsdSink) AddSample(key []string, val float32, opts ...utils.OptionExtender) {
...
func (s *DogStatsdSink) SetGaugeWithLabels(key []string, val float32, labels []metrics.Label,
	...
	flatKey, tags := s.getFlattenKeyAndCombinedLabels(key, labels)
	rate := 1.0
...
func (s *DogStatsdSink) getFlattenKeyAndCombinedLabels(key []string, labels []metrics.Label) (string, []string) {
