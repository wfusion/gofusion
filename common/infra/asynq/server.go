...
func NewServer(r RedisConnOpt, cfg Config) *Server {
	...
	return &Server{
		...
		broker:                rdb,
		state:                 srvState,
		disableRedisConnClose: cfg.DisableRedisConnClose,
		forwarder:             forwarder,
		processor:             processor,
		syncer:                syncer,
		heartbeater:           heartbeater,
...
func (srv *Server) Start(handler Handler) error {
	...
	srv.logger.Info("[Common] asynq starting processing")
...
func (srv *Server) Shutdown() {
	...
	srv.logger.Info("[Common] asynq starting graceful shutdown")
	...
	if !srv.disableRedisConnClose {
		_ = srv.broker.Close()
	}
	srv.logger.Info("[Common] asynq exiting")
}
...
func (srv *Server) Stop() {
	...
	srv.logger.Info("[Common] asynq stopping processor")
	srv.processor.stop()
	srv.logger.Info("[Common] asynq processor stopped")
}
