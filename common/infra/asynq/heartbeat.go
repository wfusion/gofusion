...
func (h *heartbeater) shutdown() {
	h.logger.Debug("[Common] asynq heartbeater shutting down...")
	...
	go func() {
		...
		for {
			...
			case <-h.done:
				...
				h.logger.Debug("[Common] asynq heartbeater done")
				timer.Stop()
				return
...
func (h *heartbeater) beat() {
	...
	if err := h.broker.WriteServerState(&info, ws, h.interval*2); err != nil {
		h.logger.Errorf("[Common] asynq failed to write server state data: %v", err)
	}

	for qname, ids := range idsByQueue {
		expirationTime, err := h.broker.ExtendLease(qname, ids...)
		if err != nil {
			h.logger.Errorf("[Common] asynq failed to extend lease for tasks %v: %v", ids, err)
			continue
		}
		for _, id := range ids {
			if l := h.workers[id].lease; !l.Reset(expirationTime) {
				h.logger.Warnf("[Common] asynq lease reset failed for %s; lease deadline: %v", id, l.Deadline())
			}
		}
	}
}
