...
func UniqueKey(k string) Option {
	return uniqueKeyOption(k)
}

func (u uniqueKeyOption) String() string   { return fmt.Sprintf("UniqueKey(%s)", u) }
func (u uniqueKeyOption) Type() OptionType { return UniqueKeyOpt }
func (u uniqueKeyOption) Value() any       { return u }
...
//
// EnqueueContext returns TaskInfo and nil error if the task is enqueued successfully,
...
// If no ProcessAt or ProcessIn options are provided, the task will be pending immediately.
//
// The first argument context applies to the enqueue operation. To specify task timeout and deadline,
// use Timeout and Deadline option instead.
func (c *Client) EnqueueContext(ctx context.Context, task *Task, opts ...Option) (info *TaskInfo, err error) {
	if task == nil {
	...
	if opt.processAt.After(now) {
		...
		err = c.addToGroup(ctx, msg, uniqueKey, opt.group, opt.uniqueTTL)
	...
	info = newTaskInfo(msg, state, opt.processAt, nil)
	switch {
	case errors.Is(err, errors.ErrDuplicateTask):
		return info, fmt.Errorf("%w", ErrDuplicateTask)
	case errors.Is(err, errors.ErrTaskIdConflict):
		return info, fmt.Errorf("%w", ErrTaskIDConflict)
	case err != nil:
		return info, err
	}
...
func (c *Client) addToGroup(ctx context.Context, msg *base.TaskMessage,
	uniqueKey, group string, uniqueTTL time.Duration) error {
	if uniqueTTL > 0 {
		return c.broker.AddToGroupUnique(ctx, msg, uniqueKey, group, uniqueTTL)
	}
