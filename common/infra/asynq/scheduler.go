...
type Scheduler struct {
	...
	preEnqueueFunc        func(task *Task, opts []Option) (err error)
...
func NewScheduler(r RedisConnOpt, opts *SchedulerOpts) *Scheduler {
	...
	return &Scheduler{
		...
		client:                NewClient(r),
		rdb:                   rdb.NewRDB(c),
		disableRedisConnClose: opts.DisableRedisConnClose,
...
func (j *enqueueJob) Run() {
	...
	if j.preEnqueueFunc != nil {
		err = j.preEnqueueFunc(j.task, j.opts)
	}
	if err == nil {
...
func (s *Scheduler) Start() error {
	...
	s.logger.Info("[Common] asynq scheduler starting")
	s.logger.Infof("[Common] asynq scheduler timezone is set to %v", s.location)
	s.cron.Start()
	s.wg.Add(1)
	go s.runHeartbeater()
	return nil
...
func (s *Scheduler) Shutdown() {
	...
	s.logger.Info("[Common] asynq scheduler shutting down")
	...
	if !s.disableRedisConnClose {
		_ = s.client.Close()
		_ = s.rdb.Close()
	}

	s.logger.Info("[Common] asynq scheduler stopped")
}
...
func (s *Scheduler) runHeartbeater() {
	...
	for {
		select {
		case <-s.done:
			s.logger.Debugf("[Common] asynq scheduler heatbeater shutting down")
...
func (s *Scheduler) beat() {
	...
	s.logger.Debugf("[Common] asynq writing entries %v", entries)
	if err := s.rdb.WriteSchedulerEntries(s.id, entries, 5*time.Second); err != nil {
		s.logger.Warnf("[Common] asynq scheduler could not write heartbeat data: %v", err)
	}
}
...
func (s *Scheduler) clearHistory() {
	for _, entry := range s.cron.Entries() {
		...
		if err := s.rdb.ClearSchedulerHistory(job.id.String()); err != nil {
			s.logger.Warnf("[Common] asynq could not clear scheduler history for entry %q: %v",
				job.id.String(), err)
		}
	}
}
