...
func (srv *Server) waitForSignals() {
	srv.logger.Info("[Common] asynq send signal TSTP to stop processing new tasks")
	srv.logger.Info("[Common] asynq send signal TERM or INT to terminate the process")
...
func (s *Scheduler) waitForSignals() {
	s.logger.Info("[Common] asynq send signal TERM or INT to stop the scheduler")
	sigs := make(chan os.Signal, 1)
