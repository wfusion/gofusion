...
func (a *aggregator) shutdown() {
	...
	a.logger.Debug("[Common] asynq aggregator shutting down...")
...
func (a *aggregator) start(wg *sync.WaitGroup) {
	...
	go func() {
		...
		for {
			select {
			case <-a.done:
				a.logger.Debug("[Common] asynq waiting for all aggregation checks to finish...")
				// block until all aggregation checks released the token
				for i := 0; i < cap(a.sema); i++ {
					a.sema <- struct{}{}
				}
				a.logger.Debug("[Common] asynq aggregator done")
				ticker.Stop()
				return
			case t := <-ticker.C:
				a.exec(t)
...
func (a *aggregator) exec(t time.Time) {
	...
	default:
		...
		a.logger.Warnf("[Common] asynq max number of aggregation checks in flight. Skipping")
	}
}
...
func (a *aggregator) aggregate(t time.Time) {
	...
	for _, qname := range a.queues {
		...
		if err != nil {
			a.logger.Errorf("[Common] asynq failed to list groups in queue: %q", qname)
			continue
		}
		for _, gname := range groups {
			...
			if err != nil {
				a.logger.Errorf("[Common] asynq failed to run aggregation check: queue=%q group=%q", qname, gname)
				continue
			}
			if aggregationSetID == "" {
				a.logger.Debugf("[Common] asynq no aggregation needed at this time: queue=%q group=%q", qname, gname)
				continue
			}
			...
			if err != nil {
				a.logger.Errorf("[Common] asynq failed to read aggregation set: queue=%q, group=%q, setID=%q",
			...
			if _, err := a.client.EnqueueContext(ctx, aggregatedTask, Queue(qname)); err != nil {
				a.logger.Errorf("[Common] asynq failed to enqueue aggregated task (queue=%q, group=%q, setID=%q): %v",
					qname, gname, aggregationSetID, err)
				cancel()
				continue
			}
			if err := a.broker.DeleteAggregationSet(ctx, qname, gname, aggregationSetID); err != nil {
				a.logger.Warnf("[Common] asynq failed to delete aggregation set: queue=%q, group=%q, setID=%q",
					qname, gname, aggregationSetID)
			}
			cancel()
		}
