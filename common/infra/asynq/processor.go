...
import (
	...
	asynqcontext "github.com/wfusion/gofusion/common/infra/asynq/pkg/context"
	"github.com/wfusion/gofusion/common/infra/asynq/pkg/errors"
	"github.com/wfusion/gofusion/common/infra/asynq/pkg/log"
	"github.com/wfusion/gofusion/common/infra/asynq/pkg/timeutil"
...
func (p *processor) stop() {
	p.once.Do(func() {
		p.logger.Debug("[Common] asynq processor shutting down...")
...
func (p *processor) shutdown() {
	...
	p.logger.Info("[Common] asynq waiting for all workers to finish...")
	// block until all workers have released the token
	for i := 0; i < cap(p.sema); i++ {
		p.sema <- struct{}{}
	}
	p.logger.Info("[Common] asynq all workers have finished")
}
...
func (p *processor) start(wg *sync.WaitGroup) {
	...
	go func() {
		...
		for {
			select {
			case <-p.done:
				p.logger.Debug("[Common] asynq processor done")
				return
			default:
				p.exec()
			}
...
func (p *processor) exec() {
	...
	case p.sema <- struct{}{}: // acquire token
		...
		case errors.Is(err, errors.ErrNoProcessableTask):
			p.logger.Debug("[Common] asynq all queues are empty")
		...
		case err != nil:
			if p.errLogLimiter.Allow() {
				p.logger.Errorf("[Common] asynq dequeue error: %v", err)
			}
			<-p.sema // release token
			return
		}
		...
		go func() {
			...
			case <-p.abort:
				...
				p.logger.Warnf("[Common] asynq quitting worker. task id=%s", msg.ID)
				p.requeue(lease, msg)
				return
			case <-lease.Done():
				cancel()
...
func (p *processor) requeue(l *base.Lease, msg *base.TaskMessage) {
	...
	if err != nil {
		p.logger.Errorf("[Common] asynq could not push task id=%s back to queue: %v", msg.ID, err)
	} else {
		p.logger.Infof("[Common] asynq pushed task id=%s back to queue", msg.ID)
	}
}
...
func (p *processor) markAsComplete(l *base.Lease, msg *base.TaskMessage) {
	...
	if err != nil {
		...
		p.logger.Warnf("[Common] asynq %s; will retry syncing", errMsg)
...
func (p *processor) markAsDone(l *base.Lease, msg *base.TaskMessage) {
	...
	if err != nil {
		errMsg := fmt.Sprintf("Could not remove task id=%s type=%q from %q err: %+v",
			msg.ID, msg.Type, base.ActiveKey(msg.Queue), err)
		p.logger.Warnf("[Common] asynq %s; will retry syncing", errMsg)
		p.syncRequestCh <- &syncRequest{
			fn: func() error {
...
func (p *processor) handleFailedMessage(ctx context.Context, l *base.Lease, msg *base.TaskMessage, err error) {
	...
	if msg.Retried >= msg.Retry || errors.Is(err, SkipRetry) {
		p.logger.Warnf("[Common] asynq retry exhausted for task id=%s", msg.ID)
		p.archive(l, msg, err)
	} else {
...
func (p *processor) retry(l *base.Lease, msg *base.TaskMessage, e error, isFailure bool) {
	...
	if err != nil {
		errMsg := fmt.Sprintf("Could not move task id=%s from %q to %q",
			msg.ID, base.ActiveKey(msg.Queue), base.RetryKey(msg.Queue))
		p.logger.Warnf("[Common] asynq %s; will retry syncing", errMsg)
		p.syncRequestCh <- &syncRequest{
			fn: func() error {
...
func (p *processor) archive(l *base.Lease, msg *base.TaskMessage, e error) {
	...
	if err != nil {
		errMsg := fmt.Sprintf("Could not move task id=%s from %q to %q",
			msg.ID, base.ActiveKey(msg.Queue), base.ArchivedKey(msg.Queue))
		p.logger.Warnf("[Common] asynq %s; will retry syncing", errMsg)
		p.syncRequestCh <- &syncRequest{
			fn: func() error {
...
func (p *processor) perform(ctx context.Context, task *Task) (err error) {
	defer func() {
		if x := recover(); x != nil {
			...
			p.logger.Errorf("[Common] asynq recovering from panic: %s, See the stack trace below for details:\n%s",
				x, errMsg)
...
func (p *processor) computeDeadline(msg *base.TaskMessage) time.Time {
	if msg.Timeout == 0 && msg.Deadline == 0 {
		p.logger.Errorf("[Common] asynq: internal error: "+
			"both timeout and deadline are not set for the task message: %s", msg.ID)
