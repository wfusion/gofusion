...
func (s *subscriber) shutdown() {
	s.logger.Debug("[Common] asynq subscriber shutting down...")
...
func (s *subscriber) start(wg *sync.WaitGroup) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				s.logger.Errorf("[Common] asynq cannot subscribe to cancelation channel: %v", err)
				select {
				case <-time.After(s.retryTimeout):
					continue
				case <-s.done:
					s.logger.Debug("[Common] asynq subscriber done")
					return
				}
			}
			break
