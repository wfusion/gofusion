...
func (r *recoverer) shutdown() {
	r.logger.Debug("[Common] asynq recoverer shutting down...")
...
func (r *recoverer) start(wg *sync.WaitGroup) {
	...
	go func() {
		...
		for {
			select {
			case <-r.done:
				r.logger.Debug("[Common] asynq recoverer done")
				timer.Stop()
				return
			case <-timer.C:
				r.recover()
...
func (r *recoverer) recoverLeaseExpiredTasks() {
	...
	if err != nil {
		r.logger.Warnf("[Common] asynq recoverer: could not list lease expired tasks: %v", err)
		return
	}
...
func (r *recoverer) recoverStaleAggregationSets() {
	for _, qname := range r.queues {
		if err := r.broker.ReclaimStaleAggregationSets(qname); err != nil {
			r.logger.Warnf("[Common] asynq recoverer: could not reclaim stale aggregation sets in queue %q: %v",
				qname, err)
		}
	}
}
...
func (r *recoverer) retry(msg *base.TaskMessage, err error) {
	...
	if err := r.broker.Retry(context.Background(), msg, retryAt, err.Error(), r.isFailureFunc(err)); err != nil {
		r.logger.Warnf("[Common] asynq recoverer: could not retry lease expired task: %v", err)
	}
}

func (r *recoverer) archive(msg *base.TaskMessage, err error) {
	if err := r.broker.Archive(context.Background(), msg, err.Error()); err != nil {
		r.logger.Warnf("[Common] asynq recoverer: could not move task to archive: %v", err)
	}
}
