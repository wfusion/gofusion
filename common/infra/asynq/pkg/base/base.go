...
import (
	...
	pb "github.com/wfusion/gofusion/common/infra/asynq/pkg/proto"
)
...
func EncodeServerInfo(info *ServerInfo) ([]byte, error) {
	...
	if err := started.CheckValid(); err != nil {
		return nil, err
	}
...
func DecodeServerInfo(b []byte) (*ServerInfo, error) {
	...
	startTime := pbmsg.GetStartTime().AsTime()
	if err := pbmsg.GetStartTime().CheckValid(); err != nil {
		return nil, err
	}
	return &ServerInfo{
		Host:              pbmsg.GetHost(),
...
func DecodeWorkerInfo(b []byte) (*WorkerInfo, error) {
	...
	startTime := pbmsg.GetDeadline().AsTime()
	if err := pbmsg.GetStartTime().CheckValid(); err != nil {
		return nil, err
	}

	deadline := pbmsg.GetDeadline().AsTime()
	if err := pbmsg.GetDeadline().CheckValid(); err != nil {
		return nil, err
	}
	return &WorkerInfo{
		Host:     pbmsg.GetHost(),
...
func EncodeSchedulerEntry(entry *SchedulerEntry) ([]byte, error) {
	...
	if err := next.CheckValid(); err != nil {
		return nil, err
	}
	prev := timestamppb.New(entry.Prev)
	if err := prev.CheckValid(); err != nil {
		return nil, err
	}
...
func DecodeSchedulerEntry(b []byte) (*SchedulerEntry, error) {
	...
	next := pbmsg.GetNextEnqueueTime().AsTime()
	if err := pbmsg.GetNextEnqueueTime().CheckValid(); err != nil {
		return nil, err
	}
	prev := pbmsg.GetPrevEnqueueTime().AsTime()
	if err := pbmsg.GetPrevEnqueueTime().CheckValid(); err != nil {
		return nil, err
	}
...
func EncodeSchedulerEnqueueEvent(event *SchedulerEnqueueEvent) ([]byte, error) {
	...
	if err := enqueuedAt.CheckValid(); err != nil {
		return nil, err
	}
...
func DecodeSchedulerEnqueueEvent(b []byte) (*SchedulerEnqueueEvent, error) {
	...
	enqueuedAt := pbmsg.GetEnqueueTime().AsTime()
	if err := pbmsg.GetEnqueueTime().CheckValid(); err != nil {
		return nil, err
	}
