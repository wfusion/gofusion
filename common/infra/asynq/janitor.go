...
func (j *janitor) shutdown() {
	j.logger.Debug("[Common] asynq janitor shutting down...")
...
func (j *janitor) start(wg *sync.WaitGroup) {
	...
	go func() {
		...
		for {
			select {
			case <-j.done:
				j.logger.Debug("[Common] asynq janitor done")
				return
			case <-timer.C:
				j.exec()
...
func (j *janitor) exec() {
	for _, qname := range j.queues {
		if err := j.broker.DeleteExpiredCompletedTasks(qname); err != nil {
			j.logger.Errorf("[Common] asynq failed to delete expired completed tasks from queue %q: %v",
				qname, err)
		}
	}
}
