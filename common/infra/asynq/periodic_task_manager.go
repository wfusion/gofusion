...
func (mgr *PeriodicTaskManager) ID() (id string) {
	if mgr == nil {
		return
	}
	return mgr.s.id
...
func (mgr *PeriodicTaskManager) Start() error {
	...
	go func() {
		...
		for {
			select {
			case <-mgr.done:
				mgr.s.logger.Debugf("[Common] asynq stopping syncer goroutine")
				ticker.Stop()
				return
			case <-ticker.C:
				mgr.sync()
...
func (mgr *PeriodicTaskManager) Run() error {
	...
	mgr.Shutdown()
	mgr.s.logger.Debugf("[Common] asynq periodic task manager exiting")
	return nil
}
...
func (mgr *PeriodicTaskManager) add(configs []*PeriodicTaskConfig) {
	for _, c := range configs {
		...
		if err != nil {
			mgr.s.logger.Errorf("[Common] asynq failed to register periodic task: cronspec=%q task=%q",
				c.Cronspec, c.Task.Type())
			continue
		}
		mgr.m[c.hash()] = entryID
		mgr.s.logger.Infof("[Common] asynq registered periodic task successfully: cronspec=%q task=%q, entryID=%s",
...
func (mgr *PeriodicTaskManager) remove(removed map[string]string) {
	for hash, entryID := range removed {
		if err := mgr.s.Unregister(entryID); err != nil {
			mgr.s.logger.Errorf("[Common] asynq failed to unregister periodic task: %v", err)
			continue
		}
		delete(mgr.m, hash)
		mgr.s.logger.Infof("[Common] asynq unregistered periodic task successfully: entryID=%s", entryID)
	}
}
...
func (mgr *PeriodicTaskManager) sync() {
	configs, err := mgr.p.GetConfigs()
	if err != nil {
		mgr.s.logger.Errorf("[Common] asynq failed to get periodic task configs: %v", err)
		return
	}
	for _, c := range configs {
		if err := validatePeriodicTaskConfig(c); err != nil {
			mgr.s.logger.Errorf("[Common] asynq failed to sync: GetConfigs returned an invalid config: %v", err)
			return
		}
	}
