...
func (f *forwarder) shutdown() {
	f.logger.Debug("[Common] asynq forwarder shutting down...")
...
func (f *forwarder) start(wg *sync.WaitGroup) {
	...
	go func() {
		...
		for {
			select {
			case <-f.done:
				f.logger.Debug("[Common] asynq forwarder done")
				return
			case <-timer.C:
				f.exec()
...
func (f *forwarder) exec() {
	if err := f.broker.ForwardIfReady(f.queues...); err != nil {
		f.logger.Errorf("[Common] asynq failed to forward scheduled tasks: %v", err)
	}
}
