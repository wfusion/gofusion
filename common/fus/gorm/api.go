...
var rootCmd = &cobra.Command{
	Use:   "gorm",
	Short: "A CLI for gorm gen-tool",
	Long:  `A CLI for gorm gen-tool`,
...
func init() {
	genPath = rootCmd.Flags().String("c", "", "is path for gen.yml")
	dsn = rootCmd.Flags().String("dsn", "", "consult[https://gorm.io/docs/connecting_to_the_database.html]")
	db = rootCmd.Flags().String("db", "mysql", "input mysql|postgres|sqlite|sqlserver|clickhouse. consult[https://gorm.io/docs/connecting_to_the_database.html]")
	tableList = rootCmd.Flags().String("tables", "", "enter the required data table or leave it blank")
	onlyModel = rootCmd.Flags().Bool("onlyModel", false, "only generate models (without query file)")
	outPath = rootCmd.Flags().String("outPath", "./dao/query", "specify a directory for output")
	outFile = rootCmd.Flags().String("outFile", "", "query code file name, default: gen.go")
	withUnitTest = rootCmd.Flags().Bool("withUnitTest", false, "generate unit test for query code")
	modelPkgName = rootCmd.Flags().String("modelPkgName", "", "generated model code's package name")
	fieldNullable = rootCmd.Flags().Bool("fieldNullable", false, "generate with pointer when field is nullable")
	fieldWithIndexTag = rootCmd.Flags().Bool("fieldWithIndexTag", false, "generate field with gorm index tag")
	fieldWithTypeTag = rootCmd.Flags().Bool("fieldWithTypeTag", false, "generate field with gorm column type tag")
	fieldSignable = rootCmd.Flags().Bool("fieldSignable", false, "detect integer field's unsigned type, adjust generated data type")
}
