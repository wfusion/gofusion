...
func configureAmqpCmd() {
	...
	utils.MustSuccess(amqpCmd.MarkPersistentFlagRequired("uri"))
	utils.MustSuccess(viper.BindPFlag("amqp.uri", amqpCmd.PersistentFlags().Lookup("uri")))
	...
	utils.MustSuccess(viper.BindPFlag("amqp.durable", amqpCmd.PersistentFlags().Lookup("durable")))
	...
	utils.MustSuccess(viper.BindPFlag("amqp.produce.exchangeType", amqpCmd.PersistentFlags().Lookup("exchange-type")))
}
...
func configureConsumeCmd(consumeCmd *cobra.Command) {
	...
	utils.MustSuccess(consumeCmd.MarkPersistentFlagRequired("queue"))
	utils.MustSuccess(viper.BindPFlag("amqp.consume.queue", consumeCmd.PersistentFlags().Lookup("queue")))
	...
	utils.MustSuccess(viper.BindPFlag("amqp.consume.exchange", consumeCmd.PersistentFlags().Lookup("exchange")))
}
...
func configureProduceCmd(produceCmd *cobra.Command) {
	...
	utils.MustSuccess(produceCmd.MarkPersistentFlagRequired("exchange"))
	utils.MustSuccess(viper.BindPFlag("amqp.produce.exchange", produceCmd.PersistentFlags().Lookup("exchange")))
	...
	utils.MustSuccess(produceCmd.MarkPersistentFlagRequired("routing-key"))
	utils.MustSuccess(viper.BindPFlag("amqp.produce.routingKey", produceCmd.PersistentFlags().Lookup("routing-key")))
}
